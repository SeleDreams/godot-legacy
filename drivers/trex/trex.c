...
static void trex_expect(TRex *exp, int n){
	if((*exp->_p) != n) 
		trex_error(exp, (const wchar_t*)_SC("expected paren"));
	exp->_p++;
}
...
static TRexChar trex_escapechar(TRex *exp)
{
	...
	} else if(!scisprint(*exp->_p)) trex_error(exp,(const wchar_t*)_SC("letter expected"));
	return (*exp->_p++);
}
...
static int trex_charnode(TRex *exp,TRexBool isclass)
{
	...
	if(*exp->_p == TREX_SYMBOL_ESCAPE_CHAR) {
		...
		trex_error(exp,(const wchar_t*)_SC("letter expected"));
	}
	t = *exp->_p; exp->_p++; 
	return trex_newnode(exp,t);
}
static int trex_class(TRex *exp)
{
	...
	if(*exp->_p == ']') trex_error(exp,(const wchar_t*)_SC("empty class"));
	chain = ret;
	while(*exp->_p != ']' && exp->_p != exp->_eol) {
		if(*exp->_p == '-' && first != -1){ 
			int r,t;
			if(*exp->_p++ == ']') trex_error(exp,(const wchar_t*)_SC("unfinished range"));
			r = trex_newnode(exp,OP_RANGE);
			if(first>*exp->_p) trex_error(exp,(const wchar_t*)_SC("invalid range"));
			if(exp->_nodes[first].type == OP_CCLASS) trex_error(exp,(const wchar_t*)_SC("cannot use character classes in ranges"));
...
static int trex_parsenumber(TRex *exp)
{
	...
	while(isdigit(*exp->_p)) {
		ret = ret*10+(*exp->_p++-'0');
		if(positions==1000000000) trex_error(exp,(const wchar_t*)_SC("overflow in numeric constant"));
		positions *= 10;
	};
	return ret;
}
...
static int trex_element(TRex *exp)
{
...

	{
		...
		switch(*exp->_p){
			...
			case '{':
				exp->_p++;
				if(!isdigit(*exp->_p)) trex_error(exp,(const wchar_t*)_SC("number expected"));
			...
			case ',':
				...
				break;
			default:
				trex_error(exp,(const wchar_t*)_SC(", or } expected"));
		}
		/*******************************/
		isgreedy = TRex_True; 
		break;
...
TRex *trex_compile(const TRexChar *pattern,const TRexChar **error)
{
	...
	exp->_p = pattern;
	exp->_nallocated = (int)scstrlen((const char*)pattern) * sizeof(TRexChar);
	...
	if(setjmp(*((jmp_buf*)exp->_jmpbuf)) == 0) {
		...
		if(*exp->_p!='\0')
			trex_error(exp,(const wchar_t*)_SC("unexpected character"));
#ifdef _DEBUG
		{
			int nsize,i;
			TRexNode *t;
...
TRexBool trex_match(TRex* exp,const TRexChar* text)
{
	...
	exp->_bol = text;
	exp->_eol = text + scstrlen((const char*)text);
	exp->_currsubexp = 0;
...
{
	return trex_searchrange(exp,text,text + scstrlen((const char*)text),out_begin,out_end);
}
